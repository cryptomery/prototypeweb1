...
import { matches as defaultMatches } from '@/data/leagueData';
...
export default function HeroSection() {
  ...
  const [matchStats, setMatchStats] = useState({ played: 0, goals: 0 });

  useEffect(() => {
    const computeStats = () => {
      try {
        const stored = localStorage.getItem('admin_matches');
        const matchList = stored ? JSON.parse(stored) : defaultMatches;
        const completed = matchList.filter((m: any) => m.status === 'completed');
        const played = completed.length;
        const goals = completed.reduce((acc: number, m: any) => acc + (Number(m.homeScore) || 0) + (Number(m.awayScore) || 0), 0);
        setMatchStats({ played, goals });
      } catch {
        const completed = defaultMatches.filter(m => m.status === 'completed');
        const played = completed.length;
        const goals = completed.reduce((acc, m) => acc + (m.homeScore ?? 0) + (m.awayScore ?? 0), 0);
        setMatchStats({ played, goals });
      }
    };
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === 'admin_league_config' || e.key === 'admin_matches') refresh();
    };
      ...
      <div className="max-w-7xl mx-auto px-6 w-full relative z-10">
        <div className="max-w-3xl space-y-8">
          ...
          <div className="flex flex-wrap gap-8 pt-6 border-t border-border/50">
            ...
            <div>
              <p className="text-3xl font-black text-foreground">{matchStats.played}</p>
              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mt-1">Pertandingan Selesai</p>
            </div>
            <div>
              <p className="text-3xl font-black text-foreground">{matchStats.goals}</p>
