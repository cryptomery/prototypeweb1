        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            {recentCompleted && recentCompleted.length > 0 ? (
                ...
                return (
                  <div key={match?.id} className="bg-card border border-border rounded-2xl p-5 match-card-hover">
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <span className="flex-1 text-sm font-bold text-foreground text-right truncate">{homeName}</span>
                      <div className="flex items-center gap-2 bg-muted rounded-xl px-4 py-2 flex-shrink-0">
                      ...
                      <span className="flex-1 text-sm font-bold text-foreground truncate">{awayName}</span>
                    </div>
                    <p className="text-[10px] text-muted-foreground/60 text-center mt-3 font-medium truncate">
                      {match?.venue}
                    </p>
                  </div>
                );
              ...
              <p className="text-xs text-muted-foreground py-4">Belum ada pertandingan yang selesai.</p>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            {upcomingMatches && upcomingMatches.length > 0 ? (
              upcomingMatches.map((match) => {
                ...
                return (
                  <div key={match?.id} className="bg-card border border-border rounded-2xl p-5 match-card-hover relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <span className="flex-1 text-sm font-bold text-foreground text-right truncate">{homeName}</span>
                      <div className="flex items-center gap-2 bg-accent/10 border border-accent/20 rounded-xl px-4 py-2 flex-shrink-0">
              ...
              <p className="text-xs text-muted-foreground py-4">Belum ada jadwal pertandingan mendatang.</p>
            )}
          </div>
        </div>
      </div>
