...
function MatchCard({
  ...
  const homeName = home?.shortName || home?.name || match.homeTeamId.toUpperCase();
  const awayName = away?.shortName || away?.name || match.awayTeamId.toUpperCase();
  const homeCity = home?.city || (match.group ? `Grup ${match.group}` : '');
  const awayCity = away?.city || (match.group ? `Grup ${match.group}` : '');
      ...
      <div className="px-6 py-5">
        <div className="flex items-center justify-between gap-4">
          <div className="flex-1 text-right min-w-0">
            <p className="text-[15px] font-bold text-foreground truncate">{homeName}</p>
            <p className="text-[11px] text-muted-foreground font-medium truncate">{homeCity}</p>
          </div>
          ...
          <div className="flex-1 text-left min-w-0">
            <p className="text-[15px] font-bold text-foreground truncate">{awayName}</p>
            <p className="text-[11px] text-muted-foreground font-medium truncate">{awayCity}</p>
          </div>
        </div>
...
export default function MatchFilter() {
  ...
  const { matches: allMatches, getTeamById, saveMatches } = useLeagueData();
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Virtualized match list or empty state */}
      {listItems.length === 0 ? (
