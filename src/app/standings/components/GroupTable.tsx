...
import type { Standing, Team } from '@/data/leagueData';
...
function FullGroupTable({
  ...
  group: string;
  teamsAdvance: number;
  standingsData: Record<string, Standing[]>;
  ...
  const containerHeight = Math.max(data.length * ROW_HEIGHT, ROW_HEIGHT);
  const { scrollRef, startIndex, endIndex, totalHeight } = useVirtualRows(data, ROW_HEIGHT, containerHeight);

  const renderRow = (s: Standing, i: number) => {
...
export default function GroupTable() {
  ...
  const { standings, getTeamById, config } = useLeagueData();
  const groups = config.groupNames;
  const teamsAdvance = config.teamsAdvancePerGroup;
